...
def painel_autor_login():
    ...
    with st.expander("🔐 Acesso Restrito ao Autor", expanded=False):
        senha_master = st.text_input(
            "Senha Master do Autor:", type="password", key="senha_master_login")
        ...
        if senha_master == "Johnwick10#":
            ...
            with col1:
                id_usuario = st.text_input(
                    "ID do Usuário:", key="id_autor_login")
            ...
            if st.button("⚙️ Gerar Código", key="btn_gerar_login"):
                if id_usuario:
                    ...
                    st.info(
                        f"📅 Válido de 01/{mes:02d}/{ano} até 01/{fim_mes:02d}/{fim_ano}  |  Usuário: {id_usuario.upper()}")
                else:
    ...
    if not tem_cadastro:
        ...
        with st.form("form_cadastro"):
            ...
            senha1 = st.text_input(
                "Crie sua senha de acesso:", type="password")
            senha2 = st.text_input("Confirme sua senha:", type="password")
            codigo = st.text_input(
                "Código de desbloqueio (fornecido pelo autor):")

            col1, col2 = st.columns(2)
            with col1:
                submitted = st.form_submit_button(
                    "✅ Cadastrar e Entrar", type="primary", use_container_width=True)
        ...
        st.info(
            f"🔑 Acesso válido até {fim_dt.strftime('%d/%m/%Y')} ({dias_restantes} dia(s))")
        ...
        st.info(
            "Entre em contato com o autor para obter um novo código de desbloqueio.")
        ...
        with st.form("form_renovacao"):
            ...
            with col1:
                submitted = st.form_submit_button(
                    "🔓 Renovar Acesso", type="primary", use_container_width=True)
            ...
            if submitted:
                if _hash(senha) != cfg.get("senha_hash", ""):
                    ...
                    res = _validar_codigo_desbloqueio(
                        cfg["id_usuario"], codigo)
                    if not res:
                        st.error("❌ Código de desbloqueio inválido!")
                    else:
                        inicio, fim = res
...
def painel_autor():
    ...
    senha_master = st.text_input(
        "Senha Master do Autor:", type="password", key="senha_master")
...
def extrair_remessas(texto):
    """Extrai remessas de qualquer formato"""
    texto = texto.strip()
    texto = re.sub(r'[,;\t\n\r]+', ' ', texto)
    remessas = []
...
def modulo_custo():
    """Módulo de processamento CUSTO - VERSÃO CORRIGIDA"""
    ...
    with tab1:
        ...
        arquivo_ptm = st.file_uploader("📂 Arquivo PTM (Aba PTM)", type=[
                                       'xlsx', 'xlsm'], key="ptm_custo")
        arquivo_modelo = st.file_uploader("💰 Modelo CUSTO (Obrigatório)", type=[
                                          'xlsx', 'xlsm'], key="modelo_custo")
    ...
    with tab2:
        ...
        st.info(
            "💡 Digite as remessas (separadas por vírgula, espaço ou quebra de linha)")
        remessas_texto = st.text_area("🔢 Remessas:", height=150, key="remessas_custo",
                                      placeholder="Exemplo:\n9202899470\n9202899479\n9202902088")

        if st.button("🚀 PROCESSAR CUSTO", type="primary", key="btn_processar_custo"):
            ...
            with st.spinner("Processando..."):
                try:
                    dados_origem = pd.read_excel(
                        st.session_state.arquivo_ptm_custo, sheet_name='PTM', engine='openpyxl')
                    ...
                    st.info(
                        f"📊 Remessas detectadas ({len(remessas)}): {', '.join(remessas)}")
                    ...
                    # CORREÇÃO: Converter remessas para float e também a coluna do DataFrame
                    remessas_float = [float(r) for r in remessas]
                    dados_origem[col_remessa] = pd.to_numeric(
                        dados_origem[col_remessa], errors='coerce')
                    df_filtrado = dados_origem[dados_origem[col_remessa].isin(
                        remessas_float)]

                    if df_filtrado.empty:
                        st.warning(
                            "⚠️ Nenhum registro encontrado para as remessas informadas!")
                        st.info(
                            f"🔍 Dica: Verifique se as remessas existem na planilha PTM")
                        return

                    st.success(f"✅ Encontrados {len(df_filtrado)} registros")

                    # Identificar pedidos com múltiplas remessas
                    grupo_pedidos = df_filtrado.groupby(
                        col_pedido)[col_remessa].nunique()
                    pedidos_multiplas = grupo_pedidos[grupo_pedidos > 1].index.tolist(
                    )

                    if pedidos_multiplas:
                        st.warning(
                            f"⚠️ Pedidos com múltiplas remessas: {len(pedidos_multiplas)}")
                    ...
                    # CORREÇÃO: Processar TODOS os registros, não apenas os com múltiplas remessas
                    # 1. Processar pedidos com múltiplas remessas
                    for pedido in pedidos_multiplas:
                        dados = df_filtrado[df_filtrado[col_pedido] == pedido]
                        output, nome = gerar_planilha_custo(
                            f"Pedido_{pedido}", dados, col_pedido, col_item, col_quantidade, "temp_modelo_custo.xlsx")
                        if output:
                            arquivos_gerados.append((output, nome))
                            st.write(f"✅ Gerado: {nome} ({len(dados)} itens)")

                    # 2. Processar remessas únicas (que não têm pedidos repetidos)
                    df_unicos = df_filtrado[~df_filtrado[col_pedido].isin(
                        pedidos_multiplas)]

                    # CORREÇÃO: Agrupar por remessa E por pedido
                    for remessa in remessas:
                        remessa_float = float(remessa)
                        dados_remessa = df_unicos[df_unicos[col_remessa]
                                                  == remessa_float]

                        if not dados_remessa.empty:
                            # Para cada pedido único nesta remessa
                            pedidos_unicos = dados_remessa[col_pedido].unique()

                            for pedido in pedidos_unicos:
                                dados = dados_remessa[dados_remessa[col_pedido] == pedido]
                                output, nome = gerar_planilha_custo(
                                    f"Remessa_{remessa}_Pedido_{pedido}", dados, col_pedido, col_item, col_quantidade, "temp_modelo_custo.xlsx")
                                if output:
                                    arquivos_gerados.append((output, nome))
                                    st.write(
                                        f"✅ Gerado: {nome} ({len(dados)} itens)")
                    ...
                    if len(arquivos_gerados) == 0:
                        st.error(
                            "❌ Nenhum arquivo foi gerado. Verifique os dados.")
                        return

                    st.success(
                        f"✅ {len(arquivos_gerados)} planilha(s) CUSTO gerada(s) com sucesso!")
...
def modulo_cubagem():
    """Módulo de processamento CUBAGEM - VERSÃO CORRIGIDA"""
    ...
    with tab1:
        ...
        arquivo_ptm = st.file_uploader("📂 Arquivo PTM (Aba PTM)", type=[
                                       'xlsx', 'xlsm'], key="ptm_cubagem")
        arquivo_espelho = st.file_uploader(
            "📋 Modelo/Espelho CUBAGEM (Opcional)", type=['xlsx', 'xlsm'], key="espelho_cubagem")
    ...
    with tab2:
        ...
        if st.button("🚀 PROCESSAR CUBAGEM", type="primary", key="btn_processar_cubagem"):
            if not remessas_texto:
                st.error("❌ Informe pelo menos uma remessa!")
                return
            ...
            with st.spinner("Processando..."):
                try:
                    dados_origem = pd.read_excel(
                        st.session_state.arquivo_ptm_cubagem, sheet_name='PTM', engine='openpyxl')
                    ...
                    # CORREÇÃO: Converter para numérico
                    remessas_float = [float(r) for r in remessas]
                    dados_origem[col_remessa] = pd.to_numeric(
                        dados_origem[col_remessa], errors='coerce')
                    df_filtrado = dados_origem[dados_origem[col_remessa].isin(
                    ...
                    for pedido in pedidos_repetidos:
                        dados = df_filtrado[df_filtrado[col_pedido] == pedido]
                        output, nome = gerar_planilha_cubagem(
                            f"Pedido_{pedido}", dados, colunas, arquivo_espelho_temp)
                        if output:
                            arquivos_gerados.append((output, nome))
                            st.write(f"✅ Gerado: {nome} ({len(dados)} itens)")
                    ...
                    for remessa in remessas:
                        ...
                        if not dados_remessa.empty:
                            ...
                            for pedido in pedidos_unicos:
                                dados = dados_remessa[dados_remessa[col_pedido] == pedido]
                                output, nome = gerar_planilha_cubagem(
                                    f"Remessa_{remessa}_Pedido_{pedido}", dados, colunas, arquivo_espelho_temp)
                                if output:
                                    arquivos_gerados.append((output, nome))
                                    st.write(
                                        f"✅ Gerado: {nome} ({len(dados)} itens)")
                    ...
                    st.success(
                        f"✅ {len(arquivos_gerados)} planilha(s) CUBAGEM gerada(s) com sucesso!")
...
def main():
    ...
    with st.sidebar:
        ...
        novo_tema = st.selectbox("🎨 Tema:", list(PALETAS.keys()), index=list(
            PALETAS.keys()).index(tema_selecionado))
    ...
    with tab1:
        ...
        3. Digite as remessas a processar
